...
mach_vm_address_t copy_shc_into_mem(mach_port_t target, uint32_t *shc, uint32_t count) {
    ...
    kr = mach_vm_protect(target, region, sizeof(uint32_t) * count, VM_PROT_READ | VM_PROT_WRITE | VM_PROT_COPY);
    ...
    kr = mach_vm_protect(target, region, sizeof(uint32_t) * count, VM_PROT_READ | VM_PROT_EXECUTE);
...
kern_return_t inject_dylib(mach_port_t target, char *dylib){
    ...
    kr = mach_vm_protect(target, remoteStr, 0x100 + strlen(dylib) + 1, VM_PROT_READ | VM_PROT_WRITE | VM_PROT_COPY);
    ...
    movk_ptr(dylib_movk, 0, remoteStr + 0x100);
    ...
    kr = thread_create_running(target, ARM_THREAD_STATE64, state, ARM_THREAD_STATE64_COUNT, &remoteThread);
    CHK_KR(kr, "thread_create_running");

    if (remoteThread == NULL) {
        printf("Remote thread is NULL!\n");
        return 1;
    }
